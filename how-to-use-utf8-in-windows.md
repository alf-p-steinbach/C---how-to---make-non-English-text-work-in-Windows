...
I.e. a practical solution is to use a DIY `Path` class &mdash; client code doesn’t need to know whether it’s a totally distinct class or a simple wrapper because with (5) fixed the C++11 `fs::path` based file open functions are only needed for Very Long Paths&trade; (in the Windows API represented as `wchar_t` based strings), and one can choose to simply not support them.
